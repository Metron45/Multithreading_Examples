...
odczyt= open("MyFIFO_przedziały",O_RDWR);

//petla czytajace i zapisujace przedziały
do{
//odczytanie poczatku i konca

rd = read(odczyt, &wynik_t, sizeof(wynik_t));

tmp_poczatek 	= wynik_t.poczatek_zakresu;
tmp_koniec 	= wynik_t.koniec_zakresu;

if(tmp_poczatek==tmp_koniec){break;}
...
printf("Worker Thread się zakończył ilość:%i\n",numer_znalezionych);
